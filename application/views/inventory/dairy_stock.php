...
<body>
    ...
    <div class="all-content-wrapper">
		...
		<div class="container-fluid" style="margin-top:15px;">
                ...
                <div class="row">
                    <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                        ...
                        <div style="padding-top:10px; font-size: 17px;">
                            <p>Stock Details - <?php if(isset($return_date)){ echo date('d-M-Y', strtotime( $return_date)); } ?></p>
                                         ...
                                         <?php if(isset($select_product)){ $i = 1; foreach($select_product as $row){ ?>
                                             <tr class="product_row" data-product_id="<?php echo $row->product_id; ?>" >
                                                 ...
                                                 <td><input type="text" value="" name="new_quantity" data-unit="<?php echo $row->unit; ?>" style="width: 80px; text-align:center; border: 1px solid #46c7fe;" ></td>
                                                   ...
                                                   <td> <?php if($row->remaining_qty > 0 && $row->product_type == 'raw_milk'){ ?> <button type="button" class="btn btn-primary" name="use" data-avl_stock="<?php echo + $row->remaining_qty; ?>" data-product_id="<?php echo $row->product_id; ?>" data-product_name="<?php echo $row->product_name; ?>" data-product_unit="<?php echo $row->unit; ?>"  data-product_type="<?php echo $row->product_type; ?>" style="padding:1px 12px; background-color:#f13f0a; border:1px solid #f13f0a; height:30px;">Transfer</button> <?php } ?>  </td>
                  ...
                  <!--  
                     <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                        ...
                        <div style="padding-top:10px; font-size: 17px;">
                            <p>Stock Details - <?php if(isset($return_date)){ echo date('d-M-Y', strtotime( ' -1 day '. $return_date)); } ?></p>
						...
						<div class="asset-inner" >
                                <table id="table" data-toggle="table"    data-key-events="true"   data-cookie="true"
									...
									<tr>
							    	  ...
							    	  <tbody id="tran_table">
                                         <?php if(isset($day_before_stock)){ $i = 1; foreach($day_before_stock as $row){ ?>
                                             <tr>
                                                 <td ><?php echo $i++; ?></td>
                                                 <td><p style="line-height:10px;"><?php echo $row->product_name; ?> </p></td>
                                                 
                                                 <td><?php if(isset($row->remaining_qty)){ if($row->unit == 'Pkt'){ echo number_format($row->remaining_qty); }else{ echo +$row->remaining_qty; } }else{ echo '0'; } echo ' '.$row->unit; ?>  </td>
                                                 
                                                 <td> <?php if($row->remaining_qty > 0){ ?> <button type="button" class="btn" name="use" data-avl_stock="<?php echo + $row->remaining_qty; ?>" data-product_id="<?php echo $row->product_id; ?>" data-product_name="<?php echo $row->product_name; ?>" data-product_unit="<?php echo $row->unit; ?>"  data-product_type="<?php echo $row->product_type; ?>" style="padding:1px 12px;">Use</button> <?php } ?>  </td>
            ...
            <div id="carryforward" class="modal modal-edu-general default-popup-PrimaryModal fade" role="dialog" data-backdrop="static" data-keyboard="false" style="width:460px; margin:auto; ">
                            <div class="modal-dialog" style="width:100%; border-top:4px solid #46c7fe;">
                                <div class="modal-content">
                                    <!--<div class="modal-close-area modal-close-df">
                                        <a class="close" data-dismiss="modal" href="#"><i class="fa fa-close"></i></a>
                                    </div>-->
                                    <div class="modal-close-area modal-close-df" style="right:-2px;">
                                        <a class="close" data-dismiss="modal" href="#" style="background-color:#f13f0a;"><i class="fa fa-close"></i></a>
                                    </div>
                                     <form action="<?php echo base_url(); ?>inventory/carry_forward"  method="post">
                                    <div class="modal-body" style="padding: 30px 30px; text-align:left;">
                                       
                                             <input type="hidden" value="" name="carry_from" />
                                         <input type="hidden" value=" <?php if(isset($return_date)){ echo date('Y-m-d', strtotime( $return_date)); } ?>" name="my_stock_date" />
                                             
                                             <div class="col-md-12">
                                                 <label style=" color:#46c7fe;">Avaliable Stock <span class="carry_from_product_stock_span"></span></label>
                                                 <div class="form-group">
                                                     <input type="text" name="carry_from_product_stock" value="" class="form-control" readonly>
                                                 </div>
                                             </div>
                                        
                                            <div class="col-md-12">
                                                 <label style=" color:#46c7fe;">Transfer Stock <span class="carry_from_product_stock_span"></span></label>
                                                 <div class="form-group">
                                                     <input type="number" name="transfer_qty" value="" step="0.1" class="form-control" required>
                                                 </div>
                                             </div>
                                        
                                             
                                             <div class="col-md-12 add_in_product"  >
                                                 <label style=" color:#46c7fe;">Add In</label>
                                                 <div class="form-group">
                                                     <select class="form-control" name="add_in_product">
                                                         <?php if(isset($select_product)){ foreach($select_product as $row){ ?>
                                                               <option value="<?php echo $row->product_id; ?>" data-required_milk="<?php echo $row->required_milk; ?>" data-unit="<?php echo $row->unit; ?>" ><?php echo $row->product_name; ?></option>
                                             ...
                                             <div class="col-md-12">
                                                 <label style=" color:#46c7fe;">Produced <span class="carry_product_span_name"></span></label>
                                                 <div class="form-group">
                                                     <input type="number" name="produced_qty" value="" min="0" class="form-control" required>
                                                 </div>
                                             </div>
                                    ...
                                    <div class="modal-footer">
                                        <div class="col-md-12">
                                         <div class="form-group">
                                                     <input type="submit" value="carryfarward" class="btn btn-primary">
                                                 </div>
                                      </div>
 ...
 function AllowOnlyNumbers(e) {
    ...
    <script type="text/javascript">
       ...
       $(document).on('keyup','input[name=new_quantity]',function(event){ 
        ...
        /// Carry forward tommorow stock
          $(document).on('click','button[name=use]',function(){ 
                $('#carryforward').modal('toggle');
              
                var product_id = $(this).data("product_id");
                var product_name = $(this).data("product_name");
                var product_unit = $(this).data("product_unit");
                var avl_stock = $(this).data("avl_stock");
                var product_type = $(this).data("product_type");
              
             //  alert(product_id+product_name+product_unit+avl_stock);
                $('.carry_product_span_name').text(product_name);
              
              
                $('.carry_from_product_stock_span').text(product_name+'/'+product_unit );
              
                $('input[name=carry_from_product_stock]').val(avl_stock);
              
                $('input[name=carry_from]').val(product_id);
              
                $('input[name=carry_stock]').attr('max',avl_stock);
              
                if(product_unit === 'Pkt'){
                    $('input[name=carry_stock]').attr('step',1);
                }else{
                    $('input[name=carry_stock]').attr('step',0.1);
                }
                
                $('select[name=add_in_product]').val(product_id);
           ...
           $(document).on('change','select[name=add_in_product]',function(){ 
               
              var required_milk =  $('select[name=add_in_product] option:selected').data('required_milk');
              var unit =  $('select[name=add_in_product] option:selected').data('unit');
              var product_name =    $('select[name=add_in_product] option:selected').text();
               
               $('.carry_product_span_name').text(product_name);
              if(unit === 'Pkt'){
                  $('input[name=carry_stock]').val('');
                  $('input[name=carry_stock]').attr('step',required_milk);
              }else{
                  $('input[name=carry_stock]').attr('step',0.1);
              }   
