                            ...
                            <div>
                               <?php if(isset($customer_details[0])){ ?>
                                <p style="margin-bottom:0px;"> <label style="color:#000; font-weight:500; margin-bottom:0px;">Customer Name :</label> <?php  echo $customer_details[0]->first_name.' '.$customer_details[0]->last_name; ?> (<?php  echo $customer_details[0]->atm_card_no; ?>) </p>
                                   ...
                                   <p style="margin-bottom:10px;"> <label  style="color:#000; font-weight:500;  margin-bottom:0px;">Available Ballance. : </label> &#8377; <?php  echo $customer_details[0]->balance_amount; echo json_decode($purchase_report)[0]; ?> </p>
                               <?php   } ?>
                            </div>
                               ...
                               <table id="table" data-toggle="table" data-show-columns="true" data-show-pagination-switch="true"  data-key-events="true" data-show-toggle="true"  data-cookie="true"
                                   ...
                                   <thead>
    								    <tr>
    								        ...
    								        <th>Dates</th>
                                            <?php if(isset($purchase_product_list)){ foreach($purchase_product_list as $product_list){  ?>
                                               <th><?php echo $product_list->product_name; ?></th>
                                            <?php }} ?>
    				                 ...
    				                 <tbody id="tran_table">
                                             ...
                                             <?php $i = 1; $recharge_total = 0; foreach(json_decode($purchase_report) as $row){ ?>
                                               <tr>

                                                   <td><?php echo $i++; ?></td>
                                                  <td><?php echo date('d-M-Y',strtotime($row->date)); ?></td>

                                                  <?php if(isset($purchase_product_list)){  foreach($purchase_product_list as $product_list){ $p_key = 'product'.$product_list->product_id; $p_price_key = 'product'.$product_list->product_id.'_price'; ?>
                                                  <td><?php if($row->$p_key){echo +$row->$p_key;  }else{ echo '-';}?>  <?php  if($row->$p_key){echo $product_list->unit;  }else{ echo '-';} // if($row->$p_price_key){echo 'Rs '.$row->$p_price_key;  }else{ echo '-';} ?></td>
                                                 <?php }} ?>
                                                   
                                                   
                                                 <td> <?php if($row->recharge){ ?> <i class="fa fa-rupee" style="color:#46c7fe;"></i> <?php echo $row->recharge; $recharge_total += $row->recharge; }else{ echo '- -'; }; ?></td>
                                                </tr>
                                             <?php } ?>
    							      </tbody>
                                      <tfoot>
                                           <?php foreach(json_decode($purchase_total_product_wise_asc) as $row){ ?>
                                            <tr>
                                                <td style="border:1px solid #dddddd;">Total</td>
                                                <td style="border:1px solid #dddddd;"></td>


                                                <?php if(isset($purchase_product_list)){  $total_sell = 0; foreach($purchase_product_list as $product_list){ $product_total_qty_key = 'product'.$product_list->product_id.'qty_total';  $product_total_price_key = 'product'.$product_list->product_id.'price_total'; ?>

                                                   <td style="border:1px solid #dddddd;"><?php echo +$row->$product_total_qty_key; if($row->$product_total_qty_key){ echo $product_list->unit;}  // echo json_encode($row); ?> / <?php if($row->$product_total_price_key ){ echo '&#8377;'; }  echo +$row->$product_total_price_key; $total_sell += $row->$product_total_price_key;  ?></td>
                                                <?php }} ?> 
                                      ...
                                      <!-- ======  total sell and total rechrge input ======== -->    
                                       <input type="hidden" value="<?php echo $total_sell; ?>" id="total_sell" />
                                       <input type="hidden" value="<?php  echo $recharge_total;  ?>" id="total_recharges" />
